...
define ( 'TECH_CNCT', 'technicalContacts' );
define ( 'SUPP_CNCT', 'supportContacts' );
define ( 'REG_AUTH', 'registrationAuthority' );
define ( 'SSO_SRV', 'SingleSignOnService' );
 ...
 *           Array containing the datas for DB connection
...
function executeStatement($mysqli, $r, $sql, $params) {
   ...
   if ($params != NULL && ! call_user_func_array ( array (
         $stmt,
         "bind_param" 
   ), refValues ( $params ) )) {
...
function getEntityPreviousStatus($mysqli, $idp) {
   ...
   $result = executeStatement ( $mysqli, true, "SELECT * FROM EntityDescriptors WHERE entityID = ? ORDER BY lastCheck", array (
         "s",
         $idp [ENTITY_ID] 
   ) );
   if ($result->num_rows > 0) {
      ...
      executeStatement ( $mysqli, false, "INSERT INTO EntityDescriptors (entityID, registrationAuthority, displayName, technicalContacts, supportContacts) VALUES (?, ?, ?, ?, ?)", array (
            "sssss",
            $idp [ENTITY_ID],
            $idp [REG_AUTH],
            $idp ['displayName'],
 ...
 *
 * @param array $idp
 *           Array containing the IdP entity
 * @param array $spEntityIDs
 *           containing the SPs entityID
 * @return new mysqli($dbConnection),
 */
function executeIdPchecks($idp, $spEntityIDs, $spACSurls, $dbConnection, $checkHistory = 2) {
   ...
   for($i = 0; $i < count ( $spEntityIDs ); $i ++) {
      $result = checkIdp ( $idp [SSO_SRV], $spEntityIDs [$i], $spACSurls [$i] );
      ...
      executeStatement ( $mysqli, false, "INSERT INTO EntityChecks (entityID, spEntityID, serviceLocation, acsUrls, checkHtml, httpStatusCode, checkResult, checkExec) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", array (
            "sssssisi",
            $idp [ENTITY_ID],
            $spEntityIDs [$i],
            $idp [SSO_SRV],
   ...
   executeStatement ( $mysqli, false, "UPDATE EntityDescriptors SET lastCheck = ?, currentResult = ?, previousResult = ?, updated = 1 WHERE entityID = ?", array (
         "ssss",
         date ( 'Y-m-d\TH:i:s\Z' ),
         $reason,
         $previousStatus,
...
function storeFedsIntoDb($jsonEdugainFeds, $dbConnection) {
   ...
   foreach ( $fedsList as $fed ) {
      ...
      if ($fed [RG_AUTH] === null || $fed [RG_AUTH] === '') {
         continue;
      }
      $result = executeStatement ( $mysqli, true, "SELECT * FROM Federations WHERE registrationAuthority = ?", array (
            "s",
            $fed [RG_AUTH] 
      ) );
      
      if ($result->num_rows <= 0) {
         executeStatement ( $mysqli, false, "INSERT INTO Federations (federationName, emailAddress, registrationAuthority, updated) VALUES (?, ?, ?, 1)", array (
               "sss",
               $fed ['name'],
               $fed [EMAIL],
               $fed [RG_AUTH] 
      ...
      while ( $row = $result->fetch_assoc () ) {
         executeStatement ( $mysqli, false, "UPDATE Federations SET updated = 1 WHERE registrationAuthority = ?", array (
               "s",
               $fed [RG_AUTH] 
         ) );
         
         if ($fed ['name'] !== $row ['federationName']) {
            executeStatement ( $mysqli, false, "UPDATE Federations SET federationName = ? WHERE registrationAuthority = ?", array (
                  "ss",
                  $fed ['name'],
                  $fed [RG_AUTH] 
            ) );
         ...
         if ($fed [EMAIL] !== $row ['emailAddress']) {
            executeStatement ( $mysqli, false, "UPDATE Federations SET emailAddress = ? WHERE registrationAuthority = ?", array (
                  "ss",
                  $fed [EMAIL],
                  $fed [RG_AUTH] 
            ) );
 ...
 *
 * @param String $jsonIdpList
 *           The JSON file that contains the identity providers
...
function extractIdPfromJSON($jsonIdpList) {
   ...
   foreach ( $idpsList as $idp ) {
      $idps [] = array(
            ...
            REG_AUTH => ( string ) $idp [REG_AUTH],
            SSO_SRV => ( string ) $idp ['Location'],
            'displayName' => getDisplayName ( $idp ),
            TECH_CNCT => getIdpMailContact ( $idp, 'technical' ),
            SUPP_CNCT => getIdpMailContact ( $idp, 'support' ) 
      );
   }
 ...
 *           The XML metadata that contains the identity providers
 * @return array idps[]("entityID" => "value",
 *         "registrationAuthority" => "value",
 *         "SingleSignOnService" => "value",
 *         "technicalContacts" => array(),
...
function extractIdPfromXML($metadata) {
   ...
   foreach ( $items as $idp ) {
      ...
      $idps [$count] [REG_AUTH] = ( string ) $idp->xpath ( "./*[local-name()='Extensions']/*[local-name()='RegistrationInfo']/@registrationAuthority" )[0];
      
      $idps [$count] [SSO_SRV] = ( string ) $idp->xpath ( "./*[local-name()='IDPSSODescriptor']/*[local-name()='SingleSignOnService'][@Binding='urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect']/@Location" )[0];
      
      $idpTechnicalContacts = $idp->xpath ( "./*[local-name()='ContactPerson'][@contactType='technical']" );
      
      if (! $idpTechnicalContacts) {
         $idps [$count] [TECH_CNCT] = "Technical Contact is missing";
      }
      
      $idps [$count] [TECH_CNCT] = array ();
      $techContacts = array ();
      ...
      foreach ( $suppContacts as $spCnt ) {
         $idps [$count] [SUPP_CNCT] [] = ( string ) $spCnt;
      }
   }
   return $idps;
}
 ...
 *
 * @param String $httpRedirectServiceLocation
 *           the HTTP-Redirect service location URL of an identity provider
