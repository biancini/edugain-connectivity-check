...
function computeCssClass($inputStatus) {
    if ($inputStatus == '1 - OK') {
        return 'green';
    }
...
function getEntities($mysqli) {
    ...
    while ($row = $result->fetch_assoc()) {
        $entity = array(
            ...
            'css_class' => ($row['ignoreEntity'] == 1) ? 'silver' : computeCssClass($row['currentResult']),
        );
...
function getChecks($mysqli) {
    ...
    while ($row = $result->fetch_assoc()) {
        $entity = array(
            ...
            'checkResult' => substr($row['checkResult']),
            'acsUrl' => $row['acsUrls'], 
            'serviceLocation' => $row['serviceLocation'],
            'css_class' => computeCssClass($row['checkResult']),
        );
        array_push($entities, $entity);
    }
...
function getCheckHtml($mysqli) {
    $id = getParameter('checkid', null);
    if (!$id) {
        throw new Exception("You must specify the checkid parameter.");
    }

    $sql = "SELECT acsUrls, serviceLocation, entityID, spEntityID, DATE_FORMAT(checkTime, '%d/%m/%Y at %H:%m:%s') as checkTime, checkHtml FROM EntityChecks WHERE id = ?";
